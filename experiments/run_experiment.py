...
def setup_lm(lm_name: str, api_key: str = None, cache: bool = True):
    """Configura el LLM. Soporta openai/, ollama/, ollama-cloud/."""
    kwargs = {}
    ...
    if lm_name.startswith("ollama-cloud/"):
        ...
        if not ollama_key:
            raise ValueError("Ollama Cloud requires OLLAMA_API_KEY")
        lm = dspy.LM(
            f"openai/{model}", api_key=ollama_key,
            api_base="https://ollama.com/v1", cache=cache,
        )
    else:
        if api_key:
...
def main():
    parser = argparse.ArgumentParser(description="Listwise Reranker experiment with DSPy")
    ...
    parser.add_argument("--primary-metric", type=str, default="ndcg", choices=["ndcg", "recall", "mrr", "map"])
    ...
    parser.add_argument("--load-optimized", type=str, default=None)
    ...
    trainset, valset = load_and_prepare(
        ...
        top_k_retrieval=args.top_k_retrieval, retrieval_method=args.retrieval_method,
        max_examples=args.max_examples, train_ratio=args.train_ratio,
    )
    
    # 2. Create reranker
    if args.summarized:
        program = SummarizedListwiseReranker(top_k=args.k, use_cot=True)
        print(f"\nCreated SummarizedListwiseReranker (k={args.k})")
    else:
        program = ListwiseReranker(top_k=args.k, use_cot=args.use_cot, verbose_signature=args.verbose_signature)
        print(f"\nCreated ListwiseReranker (k={args.k}, cot={args.use_cot})")
    
    # 3. Evaluate baseline
    print(f"\n{'='*60}\nEvaluating BASELINE\n{'='*60}")
    baseline_results = evaluate_reranker(
        program=program, testset=valset, k=args.k, k_values=[1, 5, 10, 20],
        num_threads=args.num_threads, show_progress=True,
    )
    ...
    if args.optimizer and not args.no_optimize:
        print(f"\n{'='*60}\nOPTIMIZING with {args.optimizer.upper()}\n{'='*60}")
        log_dir = str(results_dir / f"{tag}_logs")
        
        if args.optimizer == "mipro":
            optimized_program = optimize_with_mipro(
                program=program, trainset=trainset, valset=valset, k=args.k,
                primary_metric=args.primary_metric, auto=args.auto,
                num_threads=args.num_threads, log_dir=log_dir,
            )
        elif args.optimizer == "gepa":
            optimized_program = optimize_with_gepa(
                program=program, trainset=trainset, valset=valset, k=args.k,
                primary_metric=args.primary_metric, auto=args.auto,
                max_metric_calls=args.max_metric_calls, reflection_model=args.reflection_lm,
                num_threads=args.num_threads, log_dir=log_dir,
            )
        
        save_optimized_program(optimized_program, str(results_dir / f"{tag}_optimized.json"))
        
        # 5. Evaluate optimized
        print(f"\n{'='*60}\nEvaluating OPTIMIZED\n{'='*60}")
        optimized_results = evaluate_reranker(
            program=optimized_program, testset=valset, k=args.k, k_values=[1, 5, 10, 20],
            num_threads=args.num_threads, show_progress=True,
        )
        ...
        # 6. Compare
        print(compare_results(baseline_results, optimized_results))
        with open(results_dir / f"{tag}_comparison.txt", "w") as f:
            f.write(compare_results(baseline_results, optimized_results))
